...
test('DualFuel test', async ({ page }) => {
    ...
    const dualFuelBucket = parse(fs.readFileSync("src/testdata/testbuckets/Post/Warmer Home Plan - Multi - Elec Only - DD - Post.csv"), {
        columns: true,
    ...
    if (newDualFuelBucketData.length) {
        ...
        fs.writeFile('CSV Output/WHP Multi Elec Only DD POST.csv', csvFromArrayOfObjects, err => {
            if (err) {
                console.log(18, err);
            }
